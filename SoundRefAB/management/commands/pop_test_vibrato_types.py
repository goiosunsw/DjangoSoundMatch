...
class Command(BaseCommand):
    args = ''
    help = 'Build scenario for a vibrato comparison exp'

    def handle(self, *args, **options):
        # Create the scenario
        ss = Scenario(description= 'Vibrato comparison test '+ str(datetime.datetime.now()),
                      created_date = datetime.datetime.now())
        ss.save()
        
        # Create all the info pages
        
        itemdata = ({'model': Experiment,
                     'description': 'Compare vibato types for testing',
